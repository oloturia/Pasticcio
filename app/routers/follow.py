...
async def follow_user(
    ...
    # Save scalar values before flush() — after flush SQLAlchemy marks
    # ORM objects as "expired", so accessing their attributes would trigger
    # a synchronous DB reload which fails in an async context.
    target_id = target.id
    target_ap_id = target.ap_id
    current_ap_id = current_user.ap_id
    current_display = current_user.display_name or current_user.username
    current_username = current_user.username
    current_id = current_user.id

    # Build the Follow activity (stored so the Accept can reference it)
    follow_activity = _build_follow_activity(current_ap_id, target_ap_id)
    local_inbox = f"https://{settings.instance_domain}/users/{current_username}/inbox"

    follow_req = FollowRequest(
        followee_id=target_id,
        actor_ap_id=current_ap_id,
        actor_inbox=local_inbox,
        follow_activity_id=follow_activity["id"],
        is_local=True,
        requester_id=current_id,
    ...
    # Notify the target user (uses pre-extracted scalars, not expired ORM objects)
    await create_notification(
        db=db,
        recipient_id=target_id,
        notification_type=NotificationType.NEW_FOLLOWER,
        actor_ap_id=current_ap_id,
        actor_display_name=current_display,
        object_id=current_ap_id,
...
async def accept_follow_request(
    ...
    # Save scalars before flush to avoid "expired attribute" error in async context
    current_user_id = current_user.id
    current_user_ap_id = current_user.ap_id
    req_actor_ap_id = req.actor_ap_id
    req_actor_inbox = req.actor_inbox
    req_follow_activity_id = req.follow_activity_id

    # Create Follower row (idempotent — ignore duplicate)
    follower = Follower(
        followee_id=current_user_id,
        follower_ap_id=req_actor_ap_id,
        follower_inbox=req_actor_inbox,
    )
    db.add(follower)
    try:
        await db.flush()
    except IntegrityError:
        # Already a follower — rollback just this flush, then continue
        await db.rollback()

    # Update request status in a fresh flush
    ...
    follow_activity = {
        "@context": "https://www.w3.org/ns/activitystreams",
        "type": "Follow",
        "id": req_follow_activity_id or f"{req_actor_ap_id}#follow",
        "actor": req_actor_ap_id,
        "object": current_user_ap_id,
    }
    accept = build_accept_activity(current_user_ap_id, follow_activity)
    await _deliver_ap(req_actor_inbox, accept, current_user)

    # Redirect back to notifications (where the accept button lives)
    return RedirectResponse("/notifications", status_code=302)
