...
async def my_recipes_page(
    request: Request,
    tab: str = Query(default="mine"),        # mine | saved | following
    sort: str = Query(default="recent"),     # recent | alpha
    page: int = Query(default=1, ge=1),      # for "following" tab
    current_user: User | None = Depends(get_current_user_optional),
    db: AsyncSession = Depends(get_db),
):
    """
    Personal cookbook with three tabs:
      - mine:      own recipes (published + draft), sortable
      - saved:     bookmarked recipes, sortable
      - following: paginated feed from followed users
    """
    if not current_user:
        return RedirectResponse("/login", status_code=302)
    ...
    # ── Tab: mine ──────────────────────────────────────────
    own_recipes = []
    if tab == "mine" or True:   # always load for counts
        order_col = (
            Recipe.created_at.desc()
            if sort == "recent" or tab != "mine"
            else None  # handle alpha below
        )
        q = (
            select(Recipe)
            .where(
        ...
        if tab == "mine" and sort == "alpha":
            # Sort by title requires joining translations — we sort in Python
        ...
        for recipe in result.scalars().all():
            ...
            own_recipes.append({
                ...
                "dietary_tags": recipe.dietary_tags or [],
                "cover_url": f"/media/{cover.url}" if cover else None,
                "link": f"/api/v1/recipes/{recipe.id}",
            })

        if tab == "mine" and sort == "alpha":
            own_recipes.sort(key=lambda r: r["title"].lower())
    ...
    for bm in bm_result.scalars().all():
        ...
        if bm.recipe_ap_id.startswith(f"https://{settings.instance_domain}/"):
            ...
            if local_recipe:
                ...
                if cover:
                    cover_url = f"/media/{cover.url}"
                recipe_id_for_link = str(local_recipe.id)
        ...
        bookmarked.append({
            ...
            "link": (
                f"/api/v1/recipes/{recipe_id_for_link}"
                if recipe_id_for_link
                else bm.recipe_ap_id
            ),
        })

    if tab == "saved" and sort == "alpha":
        bookmarked.sort(key=lambda b: b["title"].lower())
    ...
    if followed_ids:
        ...
        all_feed = feed_result.scalars().all()
        has_more = len(all_feed) > per_page

        for recipe in all_feed[:per_page]:
            translation = next(
                (t for t in recipe.translations if t.language == recipe.original_language),
                recipe.translations[0] if recipe.translations else None,
            )
    ...
    return templates.TemplateResponse("my_cookbook.html", {
        "request": request,
        "current_user": current_user,
        "tab": tab,
        "sort": sort,
