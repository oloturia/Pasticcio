...
class FoodItem(Base):
    ...
    def get_nutrient(self, nutrient_name: str) -> float:
        ...
        Args:
            nutrient_name: e.g. "calories", "protein", "fat"
        
        Returns:
            Nutrient value or 0.0 if not available
        """
        if not self.per_100g:
            return 0.0
        return float(self.per_100g.get(nutrient_name, 0.0))
...
class Recipe(Base):
    ...
    nutrition: Mapped[list["RecipeNutrition"]] = relationship(  # noqa: F821
...
class RecipeIngredient(Base):
    ...
    food_item: Mapped["FoodItem | None"] = relationship("FoodItem", back_populates="recipe_ingredients")

    # Manual weight override in grams — used when unit cannot be auto-converted
    # (e.g. "1 piece egg" → user sets 55g manually)
    quantity_grams: Mapped[float | None] = mapped_column(Numeric(10, 3), nullable=True)

    def calculate_nutrition(self) -> dict:
        """
        Calculate nutritional values for this specific ingredient quantity.
        
        Returns:
            Dictionary with nutritional values for this ingredient
        """
        if not self.food_item or not self.food_item.per_100g or not self.quantity_grams:
            return {}
        
        # Calculate multiplier: if we have 150g and per_100g data, multiplier is 1.5
        multiplier = float(self.quantity_grams) / 100.0
        
        nutrition = {}
        for nutrient, value_per_100g in self.food_item.per_100g.items():
            nutrition[nutrient] = float(value_per_100g) * multiplier
